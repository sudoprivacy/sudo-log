...
Built-in Sudowork Log console. The page is served by the gateway and uses the same origin API.
...
```http
X-API-Key: sk-8f3a2b1c9d5e7f6a4b3c2d1e8f9a0b7c
...
`error_hash` is an internal stable grouping key generated by Sudowork Log. It is used for error-group drilldown, but should not be exposed as a manual search field.
